...
export async function fetchEntries(arg:string):Promise<Entry[]> {
    try {
        const url = API_PATH_ENTRIES
            .replace(':EntryDate', encodeURIComponent(dayjs(arg).format('YYYY-MM-DD')));
        const res = await fetchJSON<{result: Entry[]}>(url, {cache: 'no-cache'});
...
export async function postEntry(arg:BasicEntry):Promise<Entry|null> {
    try {
        const body = JSON.stringify(arg);
        const res = await fetchJSON<{result: Entry[]}>(API_PATH_SAVE_ENTRY, {method: 'POST', body});
